...
void signal_iface(int signum)
{
    syslog (LOG_NOTICE, "Vlan_tagger catch signal w/ id %d", signum);
    iface = 1;
    syslog (LOG_NOTICE, "Vlan_tagger update ifaces");
    return;
}
...
int vlan_tegger()
{
    ...
    struct sigaction action_iface;

    memset(&action, 0, sizeof(action));
    memset(&action_rules, 0, sizeof(action_rules));
    memset(&action_rules, 0, sizeof(action_iface));

    action.sa_handler = signal_handler;
    action_rules.sa_handler = signal_rule;
    action_iface.sa_handler = signal_iface;

    sigaction(SIGTERM, &action, NULL);
    sigaction(SIGUSR1, &action_rules, NULL);
    sigaction(SIGUSR2, &action_rules, NULL);
